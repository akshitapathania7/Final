...
def Natural_Language_Query_Search():
    ...
    def rank_headlines(query, headlines):
        primer = """
        As a news headline assistant, your role is to respond to user queries by identifying and presenting the most relevant news headlines. Follow these guidelines:
        ...
        4. Display the headlines in a ranked list based on their relevance to the query, with the most relevant headline appearing first, give no more than 5.
        ...
        Your responses should be concise, informative, and directly aligned with the user's interests as expressed in their query.

        Format of the result should be like below:
        1. [Most relevant headline]
        2. [Second most relevant headline]
        3. [Third most relevant headline]
        ... and so on.
        """
        prompt = f"""
        ...
        # Adding each headline as a bullet point
        for headline in headlines:
            prompt += f"- {headline}\n"
        # prompt = f"headlines relevant to the query: '{query}'. Headlines: " + " | ".join(headlines)
        ...
        # ranked_headlines_list = ranked_headlines_str.splitlines()
        # return ranked_headlines_list

        ranked_headlines_str = response.choices[0].message.content.strip()
        # Split the response into lines and then strip off the numbering from each headline
        # ranked_headlines_list = [line.split('. ', 1)[1] if '. ' in line else line for line in ranked_headlines_str.splitlines()]
    ...
    def main():
        ...
        if 'ranked_headlines' in st.session_state:
            ...
            # st.write(st.session_state.headlines)
            selected_news = st.session_state.headlines.get(selected_headline)
            # st.write(st.session_state.selected_news)
            if selected_news:
                st.subheader('Content:')
                # Use a container to control the width and display the content uniformly
                with st.container():
                    #st.write(selected_news['content'])
                    st.markdown(f'<div id="customContent">{selected_news["content"]}</div>', unsafe_allow_html=True)
                # Check if the image exists or if a placeholder text was found
                if selected_news['image']:
                    try:
                        ...
                        st.write(f"An error occurred while fetching the image.")
